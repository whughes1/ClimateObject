...
climate$methods(get_climate_data_objects = function(data_info= list()) {
  ...
  # If no station specified, return all climate_data object by default.
                                          ...
                                          start_point = length(used_data_objects)+1) 
{
  ...
  for(curr_var in vars) {
    # fix issue with season label
    if(curr_var == season_label) next
    new_col = c()
                                             ...
                                             replace=FALSE)
{
  ...
  if(compare_time_periods(time_period,data_obj$data_time_period)) {
    ...
    if( !missing(label) && summarized_obj$is_present(label) ) {
      ...
      else message(paste0(col_name," already exists. It will not be replaced."))
    }
...
climate$methods(add_end_rain_season = function(data_list=list(), earliest_day = 228, water_balance_col_name = "Water Balance", 
                                               end_rain_col_name = "End of Rain Season", capacity_max = 100, evaporation = 5,
                                               replace=FALSE) {
  ...
  for(data_obj in climate_data_objs) {
    ...
    # We can't assume year column is present
    if( !(data_obj$is_present(year_label)) ) {
      data_obj$add_year_month_day_cols()
    }
    year_col = data_obj$getvname(year_label)
    
    # We can't assume doy column is present
    ...
    for( curr_data in curr_data_list ) {
      ...
      # Remove columns not needed before splitting
      # Better to do all subsetting here before the loop
      curr_data <- curr_data[curr_data[[doy_col]] >= earliest_day,c(doy_col,waterbalance_col, year_col)]
      ...
      for (single_year in  years_split)  {
        ...
        for( j in 1:nrow(single_year) ) {
          if( !is.na(single_year[[waterbalance_col]][[j]]) 
              && single_year[[waterbalance_col]][[j]] == 0 ) {  
            end_rain[i] = single_year[[doy_col]][[j]]
            break
          }
        }
        i = i + 1  
      }
      names(end_rain) <- names(years_split)
    }
    ...
    #           xlab = "Year", main = main)
    #   }
    append_to_summary(time_period = yearly_label, data_obj, end_rain, end_rain_col_name, end_of_rain_label, replace)
  }
}
)


climate$methods(add_start_rain_col = function(data_list=list(), year=c(), earliest_day=92, total_days=2, rain_total=20, dry_length=30,
  ...
  for(data_obj in climate_data_objs) {
    ...
    #if doy or year is not in the data frame, create it.
    if( !( data_obj$is_present(dos_label) && data_obj$is_present(season_label) ) ) {
      data_obj$add_doy_col()
    }
    ...
    for(curr_data in curr_data_list ) {
      ...
      years_split <- split(curr_data[,c(dos_col,rain_col)], list(as.factor(curr_data[[season_col]])))
      ...
      for( single_year in years_split ) {
        ...
        if(!dry_spell_condition) {
          start_of_rain_col[j] = sum_check(single_year, earliest_day, total_days, rain_total)[1]
        }
    ...
    append_to_summary(time_period = yearly_label, data_obj, start_of_rain_col, col_name, start_of_rain_label, replace)
  }
                                                 ...
                                                 months_list = month.abb, day_display = "Day")
{
  ...
  for(data_obj in climate_data_objs) {
    ...
    # Is there another way to display without day, month, year cols??
